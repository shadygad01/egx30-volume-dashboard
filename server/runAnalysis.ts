...
export async function runDailyAnalysis(userId?: number) {
  ...
  try {
    for (const symbol of watchlist) {
      ...
      const existingBar = await db.select().from(dailyBars).where(and(eq(dailyBars.instrumentId, instrumentId), eq(dailyBars.tradingDate, latestTradingDate))).limit(1);
      const isNewDailyBar = !existingBar[0];
      let dailyBarId = existingBar[0]?.id;
      if (!dailyBarId) {
        const [createdBar] = await db.insert(dailyBars).values({ instrumentId, tradingDate: latestTradingDate, open: latest.open, high: latest.high, low: latest.low, close: latest.close, adjustedClose: latest.close, volume: latest.volume, provider: "yahoo-free" });
        hasNewDailyBar = true;
      ...
      await db.update(accumulationZones).set({ lifecycleStatus: "historical" }).where(and(eq(accumulationZones.instrumentId, instrumentId), lt(accumulationZones.tradingDate, latestTradingDate)));
      await db.delete(accumulationZones).where(and(eq(accumulationZones.instrumentId, instrumentId), eq(accumulationZones.tradingDate, latestTradingDate)));
      if (activeZones.length) await db.insert(accumulationZones).values(activeZones.map(zone => ({ instrumentId: instrumentId!, tradingDate: latestTradingDate, intervalStart: new Date(zone.intervalStart), intervalEnd: new Date(zone.intervalEnd), lowerPrice: zone.lowerPrice, upperPrice: zone.upperPrice, volumeRatio: zone.volumeRatio, acceptanceScore: zone.acceptanceScore, narrowRangeScore: zone.narrowRangeScore, totalScore: zone.totalScore, confidence: zone.confidence, direction: zone.direction, lifecycleStatus: "active" as const, explanation: `${zone.explanation} Daily OHLCV mode; no genuine two-hour bars available.` })));
      if (isNewDailyBar) {
