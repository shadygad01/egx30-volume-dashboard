...
- [x] Investigate whether Accumulation watch and Tracked stocks show inconsistent zones or intentionally different selections for the same stock. The direction badge came from the selected best zone, but the displayed range came from the daily candle low/high; the two sections could therefore appear contradictory for the same symbol.
- [x] Fix confirmed selection ambiguity, add regression coverage, and verify the live dashboard. Tracked stocks now displays the selected zone lower/upper bounds, labels them `Zone range`, and separates day volume; 51 tests, TypeScript, build, and visual verification passed.
